      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-2 py-3 space-y-4">
          ...
          <div className="pt-3 border-t border-neutral-100">
              ...
              <div className="relative">
                ...
                {skillsSearch && (
                  <button
                    ...
                    className="absolute right-2 top-2 text-neutral-400 hover:text-red-650 cursor-pointer border-0 bg-transparent"
                  >
                    <X className="h-3 w-3" />
                  </button>
                )}
            ...
            <div className="px-1 mb-2.5 flex flex-wrap gap-1">
              {[
                { id: "all", label: "全部" },
                { id: "introduction", label: "选题" },
                { id: "methods", label: "设计" },
                { id: "results", label: "统计" },
                { id: "discussion", label: "讨论" }
              ].map((cat) => {
                const isActive = skillsCategory === cat.id;
                return (
                  <button
                    key={cat.id}
                    type="button"
                    onClick={() => setSkillsCategory(cat.id)}
                    className={`px-1.5 py-0.5 text-[10px] font-bold border transition-all cursor-pointer rounded-md ${
                      isActive
            ...
            <div className="space-y-0.5 max-h-[280px] overflow-y-auto pr-1">
              {(() => {
                ...
                if (filtered.length === 0) {
                  return (
                    <p className="text-[10px] text-neutral-400 text-center py-4 italic font-serif">无匹配的学术工具</p>
                  );
                }

                const visible = showAllSkillsSidebar ? filtered : filtered.slice(0, 6);

                return (
                  <>
                    {visible.map((skill) => {
                      const origIndex = SKILLS_DATA.findIndex(s => s.id === skill.id);
                      const displayNum = origIndex !== -1 ? origIndex + 1 : 1;
                      return (
                        <button
                          key={skill.id}
                          type="button"
                          onClick={() => handleSelectSkillFromSidebar(skill)}
                          className="w-full text-left px-2.5 py-2 hover:bg-neutral-100 text-neutral-700 hover:text-neutral-900 rounded-lg text-xs flex items-center gap-2.5 transition-all cursor-pointer group border-0 bg-transparent"
                          title={skill.description}
                        >
                          <div className="h-6 w-6 bg-neutral-50 border border-neutral-200 text-neutral-500 group-hover:bg-[#6B1724]/8 group-hover:text-[#6B1724] group-hover:border-[#6B1724]/20 flex items-center justify-center rounded-lg text-[9px] font-mono shrink-0 transition-colors">
                            {String(displayNum).padStart(2, "0")}
                          </div>
                          <div className="flex-1 min-w-0">
                            <p className="font-semibold truncate text-neutral-800 group-hover:text-[#6B1724] text-[11px] leading-tight">
                              {skill.name.split(" (")[0]}
                            </p>
                            <p className="text-[10px] text-neutral-400 truncate font-serif mt-0.5">
                              {skill.description}
                            </p>
                          </div>
                          <ArrowRight className="h-3 w-3 text-neutral-400 group-hover:text-[#6B1724] opacity-0 group-hover:opacity-100 transition-all shrink-0" />
                        </button>
                      );
                    })}
                    ...
                    {filtered.length > 6 && (
                      <button
                        type="button"
                        onClick={() => setShowAllSkillsSidebar(!showAllSkillsSidebar)}
                        className="w-full text-center py-1.5 text-[10px] text-neutral-400 hover:text-[#6B1724] hover:underline transition-colors cursor-pointer font-bold mt-1 border-0 bg-transparent"
                      >
                        {showAllSkillsSidebar ? "收起工具" : `查看更多 (${filtered.length - 6})`}
                      </button>
                    )}
                  </>
                );
